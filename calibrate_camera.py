...
# Define paths for input images and debug outputs
images_path = os.path.join(os.getcwd(), 'images')
debug_images_path = images_path + '/debug'
success_path = debug_images_path + '/success'
failure_path = debug_images_path + '/failure'
...
if not os.path.exists(debug_images_path):
    print(f"Creating debug path: {debug_images_path}")
...
for root, _, files in os.walk(images_path):
    for fname in files:
        ...
        if ret:  # If corners are found
            ...
            cv.imwrite(os.path.join(success_path, f"success_{fname}"), img)
            cv.imshow('img', img)
            cv.waitKey(500)
            success_count += 1
        else:
            ...
            cv.imwrite(os.path.join(failure_path, f"failure_{fname}"), img)
            failure_count += 1
